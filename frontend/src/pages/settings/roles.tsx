...
const ManageRolesPage: FunctionComponent<{}> = () => {
  ...
  const deleteRole = async () => {
    ...
    if (result.success) setRoles(roles.filter((role) => role.id !== isDeletingRole!.id));
  ...
  const addRole = async () => {
    ...
    if (result.success) setRoles(roles.concat(result.data));
    else toast.error(result.error!);
    setAdding(false);
  ...
  const updateRolePermission = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.success) {
      ...
      if (rp === undefined) role.permissions = [result.data, ...(role.permissions ?? [])];
  ...
  return (
    <React.Fragment>
      ...
      {roles.length > 0 && (
        <table
          className={'is-bordered is-striped is-narrow is-hoverable table'}
                    ...
                    checked={
                      role.permissions
                        ? role.permissions.find(
                            (rp) => rp.name === Constants.ROLES_PERMISSIONS.READ_ONLY,
                          )?.value
                        : false
                    }
      ...
      {showAddingRole ? (
        <React.Fragment>
          <div className='form-container'>
            ...
            <div className='control'>
              {!adding && (
                <button className='button is-primary' onClick={addRole}>
                  Add
                </button>
              )}
