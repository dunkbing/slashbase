...
const AdvancedSettings: FunctionComponent<{}> = () => {
  ...
  const updateOpenAIKey = async () => {
    const result = await apiService.updateSingleSetting(
      Constants.SETTING_KEYS.OPENAI_KEY,
      openAIKey,
    );
    if (result.success) {
  ...
  const updateOpenAIModel = async () => {
    const result = await apiService.updateSingleSetting(
      Constants.SETTING_KEYS.OPENAI_MODEL,
      openAIModel,
    );
    if (result.success) setOpenAIModel(openAIModel);
    toast.success('saved');
  };
