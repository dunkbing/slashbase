...
import { closeTab, createTab, selectTabs, setActiveTab } from '../../redux/tabsSlice';
    ...
    <div className={'tabs is-boxed ' + styles.tabs}>
      <ul>
        {tabs.map((t) => (
          <li
            key={t.id}
            className={t.isActive ? 'is-active' : ''}
            onClick={() => {
              switchToTab(t.id);
            }}
          >
            <a>
              <span>
                ...
                {t.type === TabType.DATA &&
                  `${t.metadata.schema === '' ? t.metadata.name : `${t.metadata.schema}.${t.metadata.name}`}`}
                {t.type === TabType.MODEL &&
                  `${t.metadata.schema === '' ? t.metadata.name : `${t.metadata.schema}.${t.metadata.name}`}`}
                {t.type === TabType.QUERY &&
                  `${t.metadata.queryName ? t.metadata.queryName : 'New Query'}`}
              </span>
              <span
                className={
                  'icon ' + (t.isActive ? styles.tabsCloseBtn : styles.tabsCloseBtnInActive)
                }
                onClick={(e) => {
                  handleCloseTab(e, t.id);
                }}
              >
