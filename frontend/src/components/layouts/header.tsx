...
const Header = () => {
  ...
  const onNavigate = (option: { value: string; label: string; path: string }) => {
  ...
  if (location.pathname.startsWith('/project')) currentProjectOption = String(params.id);
  else if (location.pathname.startsWith('/db')) {
    if (currentDBConnection) currentProjectOption = currentDBConnection?.projectId;
      ...
      <div className={styles.leftBtns}>
        ...
        <Link to={Constants.APP_PATHS.HOME.path}>
          <button
            className={
              `button is-dark ` +
              [styles.btn, currentProjectOption !== undefined ? styles.home : ''].join(' ')
            }
          >
            <span className='icon'>
        ...
        <div className={styles.headerCenter}>
                ...
                <button
                  className={
                    ...
                    [
                      styles.btn,
                      styles.bread,
                      currentDBOption === undefined ? styles.breadEnds : '',
                    ].join(' ')
                  }
                  aria-haspopup='true'
                      ...
                      return (
                        <React.Fragment key={x.value}>
                          <a
                            ...
                            className={`dropdown-item${x.value === currentProjectOption ? 'is-active' : ''}`}
                          >
                            {x.label}
                          </a>
          ...
          {currentProjectOption !== undefined && currentDBOption !== undefined && (
            <div className={`dropdown${isShowingDBDropDown ? 'is-active' : ''}`}>
              <div className={`dropdown-trigger`}>
                <button
                  className={
                    'button is-dark ' +
                    [styles.btn, styles.bread, styles.dbBread, styles.breadEnds].join(' ')
                  }
                  aria-haspopup='true'
                  aria-controls='dropdown-menu'
                  onClick={() => {
                      ...
                      return (
                        <React.Fragment key={x.value}>
                          <a
                            ...
                            className={`dropdown-item${x.value === currentDBOption ? 'is-active' : ''}`}
                          >
                            {x.label}
                          </a>
                          {x.value === 'home' && <hr className='dropdown-divider' />}
              ...
              {currentDBOption !== undefined && (
                <div>
                  <button
                    id='refreshBtn'
                    data-tooltip-content='Refresh data models'
                    className={'button is-dark is-small' + [styles.btn].join(' ')}
