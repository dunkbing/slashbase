...
const searchUsers = async function (
	...
	return await Request.apiInstance
		.get<PaginatedApiResult<User, number>>(
...
const addNewProjectMember = async function (
	projectId: string,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<ProjectMember>>>(
			`/project/${projectId}/members/create`,
			payload,
		)
		.then((res) => res.data);
};

const deleteProjectMember = async function (
	projectId: string,
	userId: string,
): Promise<ApiResult<undefined>> {
	if (isapiService) {
...
const getProjectMembers = async function (
	projectId: string,
...
const addNewDBConn = async function (
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<DBConnection>>>(
			"/dbconnection/create",
			dbConnPayload,
		)
		.then((res) => res.data);
...
const getAllDBConnections = async function (): Promise<
...
const getSingleDBConnection = async function (
	dbConnId: string,
): Promise<ApiResult<DBConnection>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<DBConnection>>(
...
const deleteDBConnection = async function (
	dbConnId: string,
): Promise<ApiResult<undefined>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<undefined>>(
...
const getDBConnectionsByProject = async function (
	projectId: string,
): Promise<ApiResult<Array<DBConnection>>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<Array<DBConnection>>>(
			Events.GET_DBCONNECTIONS_BYPROJECT.RESPONSE,
		);
		EventsEmit(
			Events.GET_DBCONNECTIONS_BYPROJECT.REQUEST,
			Events.GET_DBCONNECTIONS_BYPROJECT.RESPONSE,
			projectId,
		);
		return response;
...
const getDBDataModelsByConnectionId = async function (
	dbConnId: string,
...
const getDBSingleDataModelByConnectionId = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	...
	return await Request.apiInstance
		.get<ApiResult<DBDataModel>>(
			`/query/datamodel/single/${dbConnId}?schema=${schemaName}&name=${mName}`,
		)
		.then((res) => res.data);
};

const addDBSingleDataModelField = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	fieldName: string,
	...
	return await Request.apiInstance
		.post<ApiResult<DBQueryResult>>(`/query/datamodel/single/addfield`, {
			dbConnectionId: dbConnId,
			schema: schemaName,
			name: mName,
			fieldName,
...
const deleteDBSingleDataModelField = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	fieldName: string,
	...
	if (isapiService) {
		const response = responseEvent<ApiResult<DBQueryResult>>(
			Events.DELETESINGLE_DATAMODELFIELD.RESPONSE,
		);
		EventsEmit(
			Events.DELETESINGLE_DATAMODELFIELD.REQUEST,
			Events.DELETESINGLE_DATAMODELFIELD.RESPONSE,
			{ dbConnectionId: dbConnId, schema: schemaName, name: mName, fieldName },
		);
		return response;
	}
	return await Request.apiInstance
		.post<ApiResult<DBQueryResult>>(`/query/datamodel/single/deletefield`, {
			dbConnectionId: dbConnId,
			schema: schemaName,
			name: mName,
			fieldName,
...
const addDBSingleDataModelIndex = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	indexName: string,
	...
	return await Request.apiInstance
		.post<ApiResult<DBQueryResult>>(`/query/datamodel/single/addindex`, {
			dbConnectionId: dbConnId,
			schema: schemaName,
			name: mName,
			indexName,
...
const deleteDBSingleDataModelIndex = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	indexName: string,
	...
	if (isapiService) {
		const response = responseEvent<ApiResult<DBQueryResult>>(
			Events.DELETESINGLE_DATAMODELINDEX.RESPONSE,
		);
		EventsEmit(
			Events.DELETESINGLE_DATAMODELINDEX.REQUEST,
			Events.DELETESINGLE_DATAMODELINDEX.RESPONSE,
			{ dbConnectionId: dbConnId, schema: schemaName, name: mName, indexName },
		);
		return response;
	}
	return await Request.apiInstance
		.post<ApiResult<DBQueryResult>>(`/query/datamodel/single/deleteindex`, {
			dbConnectionId: dbConnId,
			schema: schemaName,
			name: mName,
			indexName,
...
const getDBDataInDataModel = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	limit: number,
...
const updateDBSingleData = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	id: string,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<CTIDResponse>>>(
			`/query/data/${dbConnId}/single`,
			{ schema: schemaName, name: mName, id, columnName, value },
		)
		.then((res) => res.data);
};

const addDBData = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	data: any,
	...
	if (isapiService) {
		...
		EventsEmit(Events.ADD_DATA.REQUEST, Events.ADD_DATA.RESPONSE, {
			dbConnectionId: dbConnId,
			schema: schemaName,
			name: mName,
			data,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<any>>>(`/query/data/${dbConnId}/add`, {
			schema: schemaName,
			name: mName,
			data,
		})
...
const deleteDBData = async function (
	dbConnId: string,
	schemaName: string,
	mName: string,
	ids: string[],
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<DBQueryResult>>>(
			`/query/data/${dbConnId}/delete`,
			{ schema: schemaName, name: mName, ids },
		)
		.then((res) => res.data);
};

const saveDBQuery = async function (
	dbConnId: string,
	name: string,
	query: string,
	queryId: string,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<DBQuery>>>(`/query/save/${dbConnId}`, {
			name,
			queryId,
			query,
		})
...
const deleteDBQuery = async function (
	queryId: string,
): Promise<ApiResult<undefined>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<undefined>>(
	...
	return await Request.apiInstance
		.delete<any, AxiosResponse<ApiResult<undefined>>>(
			`/query/delete/${queryId}`,
		)
		.then((res) => res.data);
};

const getDBQueriesInDBConn = async function (
	dbConnId: string,
): Promise<ApiResult<DBQuery[]>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<DBQuery[]>>(
		...
		EventsEmit(
			Events.GET_DBQUERIES_INDBCONNECTION.REQUEST,
			Events.GET_DBQUERIES_INDBCONNECTION.RESPONSE,
			dbConnId,
		);
		return response;
...
const getSingleDBQuery = async function (
	queryId: string,
): Promise<ApiResult<DBQuery>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<DBQuery>>(
...
const getDBHistory = async function (
	queryId: string,
	before?: number,
): Promise<PaginatedApiResult<DBQueryLog, number>> {
	if (isapiService) {
		const response = responseEvent<PaginatedApiResult<DBQueryLog, number>>(
			Events.GET_QUERYHISTORY_INDBCONNECTION.RESPONSE,
		);
		EventsEmit(
			Events.GET_QUERYHISTORY_INDBCONNECTION.REQUEST,
			Events.GET_QUERYHISTORY_INDBCONNECTION.RESPONSE,
			queryId,
			before,
		);
	...
	return await Request.apiInstance
		.get<PaginatedApiResult<DBQueryLog, number>>(
...
const runQuery = async function (
	dbConnId: string,
	query: string,
): Promise<ApiResult<DBQueryData | DBQueryResult>> {
	if (isapiService) {
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<DBQueryData | DBQueryResult>>>(
			"/query/run",
			{ dbConnectionId: dbConnId, query },
		)
		.then((res) => res.data);
};

const getSingleSetting = async function (
	name: string,
): Promise<ApiResult<any>> {
	if (isapiService) {
...
const updateSingleSetting = async function (
	name: string,
	value: string,
): Promise<ApiResult<undefined>> {
	if (isapiService) {
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<any>>>(`/setting/single`, {
			name,
			value,
		})
		.then((res) => res.data);
...
const createTab = async function (
	...
	queryId: string,
	query: string,
): Promise<ApiResult<Tab>> {
	if (isapiService) {
		const response = responseEvent<ApiResult<Tab>>(Events.CREATE_TAB.RESPONSE);
		EventsEmit(
			Events.CREATE_TAB.REQUEST,
			Events.CREATE_TAB.RESPONSE,
			dbConnectionId,
			tabType,
			mSchema,
			mName,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<Tab>>>(`/tab/create`, {
			dbConnectionId,
			tabType,
			modelschema: mSchema,
			modelname: mName,
...
const getTabsByDBConnection = async function (
	dbConnectionId: string,
	...
	return await Request.apiInstance
		.get<any, AxiosResponse<ApiResult<Array<Tab>>>>(
			`/tab/getall/${dbConnectionId}`,
		)
		.then((res) => res.data);
};

const updateTab = async function (
	dbConnectionId: string,
	tabId: string,
	tabType: TabType,
	metadata: Object,
	...
	if (isapiService) {
		...
		EventsEmit(
			Events.UPDATE_TAB.REQUEST,
			Events.UPDATE_TAB.RESPONSE,
			dbConnectionId,
			tabId,
			tabType,
			metadata,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<Tab>>>(`/tab/update`, {
			dbConnectionId,
			tabId,
			tabType,
			metadata,
...
const closeTab = async function (
	...
	return await Request.apiInstance
		.delete<any, AxiosResponse<ApiResult<any>>>(
			`/tab/close/${dbConnectionId}/${tabId}`,
		)
		.then((res) => res.data);
};

const runConsoleCommand = async function (
	dbConnectionId: string,
	cmdString: string,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<string>>>(`/console/runcmd`, {
			dbConnectionId,
			cmd: cmdString,
		})
		.then((res) => res.data);
...
const checkConnection = async function (
	...
	return await Request.apiInstance
		.get<any, AxiosResponse<ApiResult<undefined>>>(
...
const updateRolePermission = async function (
	roleId: string,
	name: string,
	value: boolean,
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<RolePermission>>>(
...
const generateSQL = async function (
	dbConnectionId: string,
	text: string,
): Promise<ApiResult<string>> {
	if (isapiService) {
	...
	return await Request.apiInstance
		.post<any, AxiosResponse<ApiResult<string>>>(`/ai/gensql`, {
			dbConnectionId,
			text,
		})
		.then((res) => res.data);
